import { z } from 'zod';
import { ListBasesResponseSchema, BaseSchemaResponseSchema, TableSchema, FieldSchema, CommentSchema, ListCommentsResponseSchema, } from './types.js';
import { enhanceAirtableError } from './enhanceAirtableError.js';
export class AirtableService {
    apiKey;
    baseUrl;
    fetch;
    constructor(apiKey = process.env.AIRTABLE_PAT || process.env.AIRTABLE_API_KEY || '', baseUrl = 'https://api.airtable.com', fetchFn = fetch) {
    ...
    async getRecord(baseId, tableId, recordId) {
        return this.fetchFromAPI(`/v0/${baseId}/${tableId}/${recordId}`, z.object({ id: z.string(), fields: z.record(z.string(), z.any()) }));
    }
    async createRecord(baseId, tableId, fields) {
        return this.fetchFromAPI(`/v0/${baseId}/${tableId}`, z.object({ id: z.string(), fields: z.record(z.string(), z.any()) }), {
            method: 'POST',
            body: JSON.stringify({ fields }),
        });
    }
    async updateRecords(baseId, tableId, records) {
        const response = await this.fetchFromAPI(`/v0/${baseId}/${tableId}`, z.object({ records: z.array(z.object({ id: z.string(), fields: z.record(z.string(), z.any()) })) }), {
            method: 'PATCH',
    ...
    async createComment(baseId, tableId, recordId, text, parentCommentId) {
        const body = { text };
        if (parentCommentId) {
            body.parentCommentId = parentCommentId;
        }
        return this.fetchFromAPI(`/v0/${baseId}/${tableId}/${recordId}/comments`, CommentSchema, {
            method: 'POST',
            body: JSON.stringify(body),
        });
    }
    async listComments(baseId, tableId, recordId, pageSize, offset) {
        const queryParams = new URLSearchParams();
